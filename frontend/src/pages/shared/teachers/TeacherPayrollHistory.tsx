...
import {
  Table, Button, Card, Statistic, Row, Col, Tag, Spin, App,
  Modal, InputNumber, DatePicker, Select, Form, Input, Space,
} from "antd";
import {
  ArrowLeftOutlined,
  PlusOutlined,
...
const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: { lectureTagColor: "blue" },
  mbcs: { lectureTagColor: "purple" },
};
...
export default function TeacherPayrollHistory({ org }: { org: OrgType }) {
  ...
  const disassociateMutation = useMutation({
    mutationFn: () => teachersService.disassociate(org, id!),
    onSuccess: () => {
      message.success("Teacher marked as no longer associated");
      queryClient.invalidateQueries({ queryKey: [org, "teacher", id] });
  ...
  const reassociateMutation = useMutation({
    mutationFn: () => teachersService.reassociate(org, id!),
    onSuccess: () => {
  ...
  const {
    ...
    isError: payrollIsError,
    error: payrollError,
    refetch: refetchPayroll,
  } = useQuery({
    queryKey: [org, "payroll", { payableId: id, payableType: "teacher" }],
    queryFn: () =>
      payrollService.getAll(org, { payableId: id, payableType: "teacher" }, 1, 1000),
    enabled: !!id,
  });
  ...
  const handleCollectDue = async () => {
    try {
      ...
      await payrollService.collectDue(org, collectDueRecord!.id, {
  ...
  const columns: ColumnsType<PayrollRecord> = [
    ...
    },
    {
      ...
      render: (count: number | null) =>
        count != null ? <Tag color={config.lectureTagColor}>{count}</Tag> : "-",
    },
    {
      title: "Date",
      ...
      render: (_: unknown, record: PayrollRecord) => (
        <Space>
          <Button
            type="link"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/${org}/payroll/${record.id}/invoice`)}
            title="View Invoice"
          />
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate(`/${org}/teachers`)}>
          Back to Teachers
        </Button>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/${org}/payroll/create?teacherId=${id}`)}
          >
            Create Payroll
          </Button>
        </div>
