...
import { SearchOutlined, FileExcelOutlined, CalendarOutlined } from "@ant-design/icons";
...
interface OrgConfig {
  ...
  classLabel: (cls: number) => string;
  secondaryFilterField: "group" | "shift";
  secondaryFilterLabel: string;
  tertiaryFilterField: "school" | "branch";
  tertiaryFilterLabel: string;
  orgName: string;
  midColumns: ColumnsType<Student>;
  exportHeaders: string[];
  buildExportRow: (s: Student, idx: number) => (string | number)[];
}

const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: {
    classOptions: UAC_CLASSES,
    classLabel: (cls) => UAC_CLASS_MAP[cls] ?? `Class ${cls}`,
    secondaryFilterField: "group",
    secondaryFilterLabel: "Groups",
    tertiaryFilterField: "school",
    tertiaryFilterLabel: "Schools",
    orgName: "UAC",
    midColumns: [
      {
        title: "Class",
        ...
        render: (cls: number) => <Tag color="blue">{UAC_CLASS_MAP[cls] ?? `Class ${cls}`}</Tag>,
      },
      {
        title: "Group",
        dataIndex: "group",
        key: "group",
        width: 120,
        render: (g?: string) => (g ? <Tag>{g}</Tag> : <Text type="secondary">—</Text>),
      },
      {
        title: "School",
        dataIndex: "school",
    ...
    exportHeaders: [
      "Serial No", "Name", "Class", "Group", "School", "Gender", "Date of Birth",
      "Guardian Name", "Contact Number", "Father Name", "Father Mobile",
      "Mother Name", "Mother Mobile", "Address", "Monthly Tuition", "Admission Fee", "Admission Date",
    ],
    buildExportRow: (s, idx) => [
      idx + 1, s.name, UAC_CLASS_MAP[s.class!] ?? `Class ${s.class}`, s.group ?? "",
      s.school ?? "", s.gender ?? "",
      s.dateOfBirth ? dayjs(s.dateOfBirth).format("DD/MM/YYYY") : "",
      s.guardianName ?? "", s.contactNumber ?? "", s.fatherName ?? "", s.fatherMobile ?? "",
      s.motherName ?? "", s.motherMobile ?? "", s.presentAddress ?? "",
      s.monthlyTuitionFee ?? 0, s.admissionFee ?? 0,
      s.admissionDate ? dayjs(s.admissionDate).format("DD/MM/YYYY") : "",
    ],
  },
  mbcs: {
    classOptions: MBCS_CLASSES,
    classLabel: (cls) => MBCS_CLASS_MAP[cls] ?? `Class ${cls}`,
    secondaryFilterField: "shift",
    secondaryFilterLabel: "Shifts",
    tertiaryFilterField: "branch",
    tertiaryFilterLabel: "Branches",
    orgName: "MBCS",
    midColumns: [
      {
        title: "Class",
        ...
        width: 120,
        sorter: (a, b) => (a.class ?? 0) - (b.class ?? 0),
        render: (cls: number) => <Tag color="blue">{MBCS_CLASS_MAP[cls] ?? `Class ${cls}`}</Tag>,
      },
      {
        title: "Shift",
        dataIndex: "shift",
        key: "shift",
        width: 100,
        render: (s?: string) => (s ? <Tag>{s}</Tag> : <Text type="secondary">—</Text>),
      },
      {
        title: "Branch",
        dataIndex: "branch",
    ...
    exportHeaders: [
      "Serial No", "Name", "Class", "Shift", "Branch", "Gender", "Date of Birth",
      "Guardian Name", "Contact Number", "Father Name", "Father Mobile",
      "Mother Name", "Mother Mobile", "Address", "Monthly Tuition", "Admission Fee", "Admission Date",
    ],
    buildExportRow: (s, idx) => [
      idx + 1, s.name, MBCS_CLASS_MAP[s.class!] ?? `Class ${s.class}`, s.shift ?? "",
      s.branch ?? "", s.gender ?? "",
      s.dateOfBirth ? dayjs(s.dateOfBirth).format("DD/MM/YYYY") : "",
      s.guardianName ?? "", s.contactNumber ?? "", s.fatherName ?? "", s.fatherMobile ?? "",
      s.motherName ?? "", s.motherMobile ?? "", s.presentAddress ?? "",
...
export default function StudentDirectory({ org }: { org: OrgType }) {
  ...
  const [tertiaryFilter, setTertiaryFilter] = useState<string | undefined>();
  ...
  const { data: studentsData, isLoading } = useQuery({
    queryKey: [org, "students-directory"],
  ...
  const availableSecondary = useMemo(
    () =>
      [
        ...new Set(
          allStudents
            .map((s) => s[config.secondaryFilterField] as string | undefined)
            .filter(Boolean) as string[],
        ),
      ].sort(),
    [allStudents, config.secondaryFilterField],
  );
  ...
  const availableTertiary = useMemo(
    () =>
      [
        ...new Set(
          allStudents
            .map((s) => s[config.tertiaryFilterField] as string | undefined)
            .filter(Boolean) as string[],
        ),
      ].sort(),
    [allStudents, config.tertiaryFilterField],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (secondaryFilter)
      result = result.filter(
        (s) => (s[config.secondaryFilterField] as string | undefined) === secondaryFilter,
      );
    if (tertiaryFilter)
      result = result.filter(
        (s) => (s[config.tertiaryFilterField] as string | undefined) === tertiaryFilter,
      );
    if (searchText) {
  ...
  const exportToExcel = useCallback(() => {
    const rows = filtered.map((s, idx) => config.buildExportRow(s, idx));
    const csvContent = [
      config.exportHeaders.join(","),
      ...rows.map((row) =>
        row
          .map((cell) => {
    ...
    const classStr =
      classFilter !== undefined
        ? `_${config.classLabel(classFilter).replace(/\s/g, "")}`
        : "";
    link.download = `${config.orgName}_Student_Directory${classStr}_${dateStr}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  }, [filtered, classFilter, config]);
  ...
  const columns: ColumnsType<Student> = [
    ...
    ...config.midColumns,
    { title: "Guardian", dataIndex: "guardianName", key: "guardianName", width: 150 },
    { title: "Contact", dataIndex: "contactNumber", key: "contactNumber", width: 140 },
    {
      title: "Tuition",
    ...
    <div>
      ...
      >
        <Title level={4} style={{ margin: 0 }}>
          {config.orgName} Student Directory
        </Title>
        <Button
          type="primary"
      ...
      <Card style={{ marginBottom: 16 }}>
        <Row gutter={[12, 12]}>
          ...
          <Col xs={24} sm={8} md={4}>
            <Select
              allowClear
              placeholder={`All ${config.secondaryFilterLabel}`}
              value={secondaryFilter}
              onChange={setSecondaryFilter}
              options={availableSecondary.map((v) => ({ value: v, label: v }))}
              style={{ width: "100%" }}
          ...
          <Col xs={24} sm={8} md={4}>
            <Select
              allowClear
              placeholder={`All ${config.tertiaryFilterLabel}`}
              value={tertiaryFilter}
              onChange={setTertiaryFilter}
              options={availableTertiary.map((v) => ({ value: v, label: v }))}
              style={{ width: "100%" }}
              disabled={availableTertiary.length === 0}
            />
          </Col>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card size="small">
            <Statistic title="Total Students" value={filtered.length} />
          </Card>
        </Col>
        <Col span={6}>
          <Card size="small">
          ...
          <Card size="small">
            <Statistic
              ...
              value={
                filtered.length > 0
                  ? `৳${Math.round(
                      filtered.reduce((s, st) => s + (st.monthlyTuitionFee ?? 0), 0) /
                        filtered.length,
                    )}`
                  : "—"
              }
        ...
        <Col span={6}>
          <Card size="small">
            <Statistic
              ...
              value={`৳${filtered
                .reduce((s, st) => s + (st.monthlyTuitionFee ?? 0), 0)
                .toLocaleString()}`}
            />
          </Card>
        </Col>
