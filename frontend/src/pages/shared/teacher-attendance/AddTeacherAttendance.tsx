...
const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: {
    classOptions: [8, 9, 10, 11, 12],
    classLabel: (cls) => `Class ${cls}`,
  },
  mbcs: {
    classOptions: [6, 7, 8, 9, 10],
...
export default function AddTeacherAttendance({ org }: { org: OrgType }) {
  ...
  const lectureBased = teachers.filter((t) => t.paymentType === "lecture_based");

  const createMutation = useMutation({
    mutationFn: (data: CreateAttendanceDto) => teacherAttendanceService.create(org, data),
    onSuccess: () => {
  ...
  const summaryMutation = useMutation({
    mutationFn: (data: { teacherId: string; month: string; totalLectures: number }) =>
      teacherAttendanceService.createMonthlySummary(org, data),
    onSuccess: () => {
    ...
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
            ...
            {mode === "simplified" ? (
              <>
                ...
                <Col span={12}>
                  <Form.Item
                    ...
                    name="totalLectures"
                    rules={[{ required: true, message: "Please enter total lectures" }]}
                  >
                    <InputNumber min={1} style={{ width: "100%" }} placeholder="Total lectures taken" />
                  </Form.Item>
                </Col>
              </>
            ) : (
              <>
                ...
                <Col span={12}>
                  ...
                  >
                    <InputNumber min={1} max={10} style={{ width: "100%" }} placeholder="Number of lectures" />
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item label="Class" name="class">
                    <Select placeholder="Which class (optional)" allowClear>
                      {config.classOptions.map((cls) => (
        ...
        <Form.Item>
          ...
          <Button
            style={{ marginLeft: 8 }}
            onClick={() => navigate(`/${org}/teacher-attendance`)}
            size="large"
          >
            Back to List
          </Button>
