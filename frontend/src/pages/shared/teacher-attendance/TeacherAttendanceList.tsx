...
interface OrgConfig {
  filterType: "dateRange" | "month";
  lectureTagColor: "blue" | "purple";
}

const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: { filterType: "dateRange", lectureTagColor: "blue" },
  mbcs: { filterType: "month", lectureTagColor: "purple" },
};

export default function TeacherAttendanceList({ org }: { org: OrgType }) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const config = ORG_CONFIG[org];
  const [attendanceMode, setAttendanceMode] = useState<"simplified" | "detailed">("simplified");
  ...
  const { data, isLoading } = useQuery({
    queryKey: [org, "teacher-attendance", filters],
    queryFn: () => teacherAttendanceService.getAll(org, filters),
  });

  const attendances: TeacherAttendance[] = data?.data || [];

  const deleteMutation = useMutation({
    mutationFn: (id: string) => teacherAttendanceService.delete(org, id),
    onSuccess: () => {
      message.success("Attendance record deleted");
      queryClient.invalidateQueries({ queryKey: [org, "teacher-attendance"] });
    },
  ...
  const columns: ColumnsType<TeacherAttendance> = [
      ...
      render: (_: unknown, record: TeacherAttendance) => (
        <div>
          ...
          <div style={{ fontSize: 12, color: "#888" }}>
            {record.teacher?.paymentType === "lecture_based" ? "Lecture Based" : "Fixed"}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      width: 100,
      render: (count: number) => (
        <Tag color={config.lectureTagColor}>
      ...
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
        <Space>
          <Select
            ...
            style={{ width: 200 }}
            onChange={(value) => setFilters((prev) => ({ ...prev, teacherId: value || undefined }))}
            allowClear
            showSearch
            options={teachers.map((t: Teacher) => ({ value: t.id, label: t.name }))}
          ...
          {config.filterType === "dateRange" ? (
            ...
            <DatePicker
              ...
              onChange={(date) =>
                setFilters((prev) => ({
                  ...prev,
                  month: date ? date.format("YYYY-MM") : undefined,
                }))
              }
              format="MMMM YYYY"
            />
        ...
        <Space>
          ...
          <Switch
            ...
            onChange={(checked) => setAttendanceMode(checked ? "detailed" : "simplified")}
          ...
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/${org}/teacher-attendance/add?mode=${attendanceMode}`)}
          >
            Record Attendance
          </Button>
        </Space>
