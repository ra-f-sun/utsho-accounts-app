...
export default function StaffPayrollHistory({ org }: { org: OrgType }) {
  ...
  const disassociateMutation = useMutation({
    mutationFn: () => staffService.disassociate(org, id!),
    onSuccess: () => {
      message.success("Staff marked as no longer associated");
      queryClient.invalidateQueries({ queryKey: [org, "staff-detail", id] });
  ...
  const reassociateMutation = useMutation({
    mutationFn: () => staffService.reassociate(org, id!),
    onSuccess: () => {
  ...
  const { data: staffData, isLoading: loadingStaff } = useQuery({
    queryKey: [org, "staff-detail", id],
    queryFn: () => staffService.getOne(org, id!),
    enabled: !!id,
  });
  ...
  const {
    ...
    queryKey: [org, "payroll", { payableId: id, payableType: "staff" }],
    queryFn: () => payrollService.getAll(org, { payableId: id, payableType: "staff" }, 1, 1000),
    enabled: !!id,
  });
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate(`/${org}/staff`)}>
          Back to Staff
        </Button>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate(`/${org}/payroll/create`)}
          >
            Create Payroll
          </Button>
        </div>
