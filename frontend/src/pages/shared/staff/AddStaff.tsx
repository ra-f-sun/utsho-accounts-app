...
const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: { designationRequired: true },
  mbcs: { designationRequired: false },
};

export default function AddStaff({ org }: { org: OrgType }) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateStaffDto) => staffService.create(org, data),
    onSuccess: () => {
      message.success("Staff added successfully");
      queryClient.invalidateQueries({ queryKey: [org, "staff"] });
      navigate(`/${org}/staff`);
    },
    onError: () => message.error("Failed to add staff"),
  });

  const updateMutation = useMutation({
    mutationFn: (data: Partial<CreateStaffDto>) => staffService.update(org, id!, data),
    onSuccess: () => {
      message.success("Staff updated successfully");
      queryClient.invalidateQueries({ queryKey: [org, "staff"] });
      navigate(`/${org}/staff`);
    },
    onError: () => message.error("Failed to update staff"),
  });
    ...
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item
                label="Designation"
                name="designation"
                rules={config.designationRequired ? [{ required: true, message: "Please enter designation" }] : []}
              >
        ...
        <Form.Item>
          ...
          <Button style={{ marginLeft: 8 }} onClick={() => navigate(`/${org}/staff`)} size="large">
            Cancel
          </Button>
        </Form.Item>
      </Form>
