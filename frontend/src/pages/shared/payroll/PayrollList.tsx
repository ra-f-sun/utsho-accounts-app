...
export default function PayrollList({ org }: { org: OrgType }) {
  const { message: msg } = App.useApp();
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [dueForm] = Form.useForm();
  const [filters, setFilters] = useState<{ payableType?: string; paymentMonth?: string }>({});
  const [page, setPage] = useState(1);
  const [collectDueTarget, setCollectDueTarget] = useState<Payroll | null>(null);

  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: [org, "payroll", filters, page],
    queryFn: () => payrollService.getAll(org, filters, page),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => payrollService.delete(org, id),
    onSuccess: () => {
  ...
  const collectDueMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: { paidAmount: number; paymentDate: string; paymentMethod: string; notes?: string };
    }) => payrollService.collectDue(org, id, data),
    onSuccess: () => {
      ...
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
        <Space>
          ...
          <Select
            ...
            style={{ width: 130 }}
            onChange={(value) => setFilters((prev) => ({ ...prev, payableType: value || undefined }))}
            allowClear
          >
            ...
            >
              ...
              >
                <InputNumber
                  style={{ width: "100%" }}
                  min={0.01}
                  max={collectDueTarget.dueAmount ?? undefined}
                  precision={2}
                  prefix="৳"
                />
              </Form.Item>
