...
type InvoiceComponent = React.ForwardRefExoticComponent<
  { data: PayrollInvoiceData } & React.RefAttributes<HTMLDivElement>
>;

interface OrgConfig {
  TeacherInvoice: InvoiceComponent;
  StaffInvoice: InvoiceComponent;
}

const ORG_CONFIG: Record<OrgType, OrgConfig> = {
  uac: { TeacherInvoice: UacTeacherPayrollInvoice, StaffInvoice: UacStaffPayrollInvoice },
  mbcs: { TeacherInvoice: MbcsTeacherPayrollInvoice, StaffInvoice: MbcsStaffPayrollInvoice },
};

export default function PayrollInvoice({ org }: { org: OrgType }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [org, "payroll-record", id],
    queryFn: () => payrollService.getOne(org, id!),
    enabled: !!id,
  });
  ...
  const { data: teacherData } = useQuery({
    queryKey: [org, "teacher", payroll?.payableId],
    queryFn: () => teachersService.getOne(org, payroll!.payableId),
    enabled: !!payroll?.payableId && payroll?.payableType === "teacher",
  });

  const { data: staffData } = useQuery({
    queryKey: [org, "staff", payroll?.payableId],
    queryFn: () => staffService.getOne(org, payroll!.payableId),
    enabled: !!payroll?.payableId && payroll?.payableType === "staff",
  });

  const payableName = teacherData?.data?.name || staffData?.data?.name || payroll?.payableType;
  ...
  return (
    <div>
      ...
      {payroll.payableType === "teacher" ? (
        <config.TeacherInvoice ref={invoiceRef} data={invoiceData} />
      ) : (
        <config.StaffInvoice ref={invoiceRef} data={invoiceData} />
      )}
    </div>
  );
}
