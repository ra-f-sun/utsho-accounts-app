...
export default function MbcsDiscountSettings() {
  ...
  async function loadSettings() {
    ...
    rows.forEach((row) => {
      ...
      if (row.settingKey === "discount_admission_options") setAdmissionDiscounts(vals);
      if (row.settingKey === "discount_readmission_options") setReadmissionDiscounts(vals);
    });
  }
  ...
  async function handleSave() {
    ...
    try {
      await settingsService.bulkUpsertSettings(ORG, [
        ...
        { key: "discount_admission_options", value: { values: admissionDiscounts } },
        { key: "discount_readmission_options", value: { values: readmissionDiscounts } },
      ]);
  ...
  return (
    <Space orientation="vertical" style={{ width: "100%" }} size="large">
      ...
      />
      <DiscountSection
        title="Admission Fee Discount Options"
        description="These values appear as a dropdown on the student form for admission fee discount."
        values={admissionDiscounts}
        onChange={setAdmissionDiscounts}
      />
      <DiscountSection
        title="Re-Admission Fee Discount Options"
        description="These values appear as a dropdown for re-admission fee discount."
        values={readmissionDiscounts}
        onChange={setReadmissionDiscounts}
      />
